...
/*void Kinect::keyPressed(unsigned char key, int x, int y)
{
	if (key == 27) {
		die = 1;
...
void Kinect::ReSizeGLScene(int Width, int Height)
{
...
void Kinect::InitGL(int Width, int Height)
{
...
void* Kinect::gl_threadfunc(void *arg)
{
	printf("GL thread\n");
...
void Kinect::modifyImage(int largeur, int longueur)
{
    ...
    cv::threshold( src, temp1, 51, 255,THRESH_TOZERO ); //51 <=> 0,7m
    cv::threshold( temp1, temp2, 95, 255,THRESH_TOZERO_INV); //95 <=> 1,3m
    erode(temp2, mask, Mat());
}

int Kinect::get_image()
{
	int res;
